...
export interface LeagueCapYear {
  ...
  /**
   * Minimum-salary CAP-HIT scale by years of service (0 = rookie). 0-1 YOS
   * carries the rookie-scale minimum; 2+ YOS is fixed at the two-year-veteran
   * minimum regardless of actual experience — the CBA's Minimum Salary
   * Exception caps what any <=2yr minimum deal counts against the cap at that
   * figure, with the league funding the gap to the player's real, higher
   * paycheck. Matches the real scraped salaries in player-data.ts, where
   * veterans at every actual experience level share this same cap number.
   */
...
// Two-tier minimum cap-hit scale: 0-1 YOS gets the rookie-scale minimum,
// 2+ YOS gets the two-year-veteran minimum (see minimumByYos doc above).
function minimumScale(rookieValue: number, vetValue: number): Record<number, number> {
  const scale: Record<number, number> = {}
  for (let yos = 0; yos <= 10; yos++) scale[yos] = yos <= 1 ? rookieValue : vetValue
  return scale
}
...
export const LEAGUE_CAP: Record<Season, LeagueCapYear> = {
  '2026-27': {
    ...
    minimumByYos: minimumScale(1320000, 2449421),
  ...
  '2027-28': {
    ...
    minimumByYos: minimumScale(1392601, 2584141),
  ...
  '2028-29': {
    ...
    minimumByYos: minimumScale(1469195, 2726271),
  ...
  '2029-30': {
    ...
    minimumByYos: minimumScale(1549998, 2876210),
  ...
  '2030-31': {
    ...
    minimumByYos: minimumScale(1635251, 3034408),
  ...
  '2031-32': {
    ...
    minimumByYos: minimumScale(1725192, 3201304),
  ...
  '2032-33': {
    ...
    minimumByYos: minimumScale(1820078, 3377377),
  ...
  '2033-34': {
    ...
    minimumByYos: minimumScale(1920182, 3563132),
  ...
  '2034-35': {
    ...
    minimumByYos: minimumScale(2025791, 3759102),
