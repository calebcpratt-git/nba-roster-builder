...
export const FIDELITY_NOTE =
  "Not checked here: Bird rights, base-year compensation & poison-pill contracts, full pick-conveyance logic for protections/swaps, the moratorium & trade-deadline timing, and the two-month re-aggregation rule. Sign-and-trades are validated, but only at creation time in the sign-and-trade flow on the free-agent signing modal (MLE-used block, seller-apron block, 3-4yr length rule, plus this same validator) — editing a saved sign-and-trade back open in the regular trade editor re-runs only the checks below, not those sign-and-trade-specific ones. Pick conditions are only consulted to soften Stepien-rule and touch-rule findings, never resolved — a protected pick is treated as not conveying for the touch rule and flagged rather than judged. No-trade clauses and trade bonuses/kickers are checked, but only for the small set of players with that data populated in CONTRACT_DETAILS. Held trade exceptions are checked against each team's live SalarySwish-sourced ledger; cash-in-trade limits against a live Hoops Rumors-sourced ledger. Both when available. Cap and apron figures are approximations for planning, not a substitute for a live trade machine. Partner-team salary includes their roster, saved contracts, and saved trades in this app — trades built from another team's perspective aren't visible here."
